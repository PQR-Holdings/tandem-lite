...
Tandem Lite is an open-source, local-first VS Code extension for planning and carrying out approved developer and Windows desktop tasks. It uses an LLM to create a short tool-based plan, shows that plan for review, and verifies each completed step where a reliable postcondition is available.
...
- Search Windows folders for files, including extension-filtered image searches, then open a selected result.
- Open known Windows folders and absolute file or folder paths in Explorer.
- Search Start-menu applications and launch a discovered application.
- List visible windows, then focus or request closure of a selected window.
- Inspect accessible Windows UI Automation controls, then focus a control, invoke it, or set an accessible text value.
...
The extension uses tool-specific policy settings. By default, actions such as terminal execution, opening Windows locations/apps, searching outside the workspace, and controlling a window/UI require approval. Permissions can be changed in the Settings panel.
...
Tandem Lite does not bypass operating-system permissions, application security controls, anti-cheat systems, or UAC elevation boundaries.
...
- Windows desktop actions are Windows-only and depend on the target application exposing standard Windows UI Automation controls.
...
- Some desktop actions can only be verified as “requested” or “inconclusive” because Windows does not always expose the resulting state to UI Automation or the shell API.
...
Requirements: Node.js and VS Code. Windows is required to test desktop tools.
...
The test suite uses Node's built-in test runner. It does not replace end-to-end testing against real Windows applications.
